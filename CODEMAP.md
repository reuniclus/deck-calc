...
| `src/state/useSuggestions.tsx` | `SuggestionsProvider`/`useSuggestionsCtx()` — computes the suggestion search ONCE per goal change and shares it, instead of AdvisorStrip/SuggestionsTab/chart each independently re-running it (measured: ~15ms fast path / ~300ms general path, so 3 independent calls was real, felt latency, not just wasted cycles — confirmed a ~5x reduction after deduping) |
