...
| `src/ui/ResultView.tsx` | status line, Chart/Table/Grid/Suggestions tabs (each with a distinct `tab-panel-*` wrapper class — Table and Suggestions both use `.num-table` internally, and sharing an ambiguous selector was a real test bug caught this session). Kept mounted (display-toggled), not conditionally rendered, so switching tabs doesn't reset a tab's own local state. Tab state now lives in `App.tsx`'s `Layout`, lifted up so `AdvisorStrip`'s "See suggestions" link can switch it |
