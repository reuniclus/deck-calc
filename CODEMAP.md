...
| `src/ui/useDebouncedCommit.ts` | local state updates instantly on every keystroke (input never feels laggy); the actual commit — which triggers the expensive recompute — only fires after a pause. Typing "90" as two keystrokes now commits once, not twice |
